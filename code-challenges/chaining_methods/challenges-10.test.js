...
const createServer = () => {
  ...
  function favoriteFoods(request, response){
    response.send(['fried rice', 'pizza', 'tacos'])
  }
...
const count = (target, input) => {
  let answer = input.reduce((accumulator, outerValue) => {
    const array = outerValue.filter(value => {
      if (value === target){
        return value;
      }
    })
...
const totalSum = (input) => {
  let sum = input.reduce((outerAccumulator, outerValue) => {
    let add = outerValue.reduce((innerAccumulator, innerValue) => {
      innerAccumulator += innerValue;
      return innerAccumulator;
    }, 0)
    outerAccumulator += add;
...
const divisibleByFiveTwoToThePower = (input) => {
  const arr = input.map(outerValue => {
    let divByFive = outerValue.filter(innerValue => {
      if(typeof innerValue === 'number'){
        
        return innerValue % 5 === 0;
      }
    })
    return divByFive;
  })
  const power = arr.map(outerValue => {
    let raisePower = outerValue.map(innerValue => {
      return Math.pow(2, innerValue);
    })
    return raisePower;
  })
  return power;
